...
const filename = `data/ibm-vpc.json`;
const RETRY_DELAY_MS = 30000;
const MAX_RETRIES = 3;
const vendor = 'ibm';
const serviceName = 'containers-kubernetes';

/**
 * Product Mapping:
 * DB:             | ibmProductJson:
 * --------------- | -----------------
 * productHash:    | md5(vendor + flavor + region + operating_system + plan_id + country + currency);
 * sku:            | plan_id
 * vendorName:     | 'ibm'
 * region:         | region
...
async function downloadAll(): Promise<void> {
  ...
  try {
    ...
    config.logger.error(
      `Skipping IBM VPC due to error ${writeErr}.`
    );
  }
}
