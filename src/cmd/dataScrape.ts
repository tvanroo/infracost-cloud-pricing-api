...
import ibmInstances from '../scrapers/ibmInstances';
...
async function run(): Promise<void> {
  const { argv } = yargs
    .usage(
      'Usage: $0 --only=[aws:bulk,aws:spot,azure:retail,gcp:catalog,gcp:machineTypes,ibm:instances]'
    )
    .options({
      only: { type: 'string' },
    });
